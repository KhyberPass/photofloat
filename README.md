...
* For scanner - Reduce the number of thumb sizes generated by the scanner, Ignore iPhone AAE files, use mediainfo for video metadata, Rotate images if the metadata says to, Dont run thumb generation as multi process, Update ffmpeg options
* For viewer - Show still thumb for video file
